...
class DafenGui(tk.Frame):
    ...
    def get_queue_info(self):
        if self.is_running or not picture_test.infobox_queue.empty():
            self.master.after(100, self.get_queue_info)
            if not picture_test.infobox_queue.empty():
                self.append_text_in_textbox(picture_test.infobox_queue.get(), True)
    ...
    def write_result_in_textbox(self):
        ...
        #t1 = Thread(target=picture_test.start_test, args=(self.source_folder,))
        #t1.start()
